...
int ERBuildOptimizer::ValidateOptimization() const {
	...
	if (using_off_hand) {
		...
		case OPTIMIZATION_TYPE::DAMAGE_PHYSICAL:
			if (oh_weapon.damage_physical > 0)
				return CALC_PROCEED;
			break;
		case OPTIMIZATION_TYPE::DAMAGE_MAGIC:
			if (oh_weapon.damage_magic > 0)
				return CALC_PROCEED;
			break;
		case OPTIMIZATION_TYPE::DAMAGE_FIRE:
			if (oh_weapon.damage_fire > 0)
				return CALC_PROCEED;
			break;
		case OPTIMIZATION_TYPE::DAMAGE_LIGHTNING:
			if (oh_weapon.damage_lightning > 0)
				return CALC_PROCEED;
			break;
		case OPTIMIZATION_TYPE::DAMAGE_HOLY:
			if (oh_weapon.damage_holy > 0)
				return CALC_PROCEED;
			break;
		case OPTIMIZATION_TYPE::STATUS_POISON:
